...
class SDAE_PART(TorchModelV2):
    def __init__(self, obs_space: gym.spaces.Space, action_space: gym.spaces.Space, num_outputs: int,
        ...
        pv = obs_space.original_space.spaces['private_vars']
        sdae_in = int(np.product(obs_space_)) - int(np.product(pv.shape))
        ...
        prev_layer_size = dim[-1] + int(np.product(pv.shape))
        layers = []
        for size in hiddens:
            layers.append(
                SlimFC(
...
ModelCatalog.register_custom_model("sdae_part", SDAE_PART)
