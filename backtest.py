...
def BackTestStats(account_value, debug=True):
    df = account_value.copy()
    df = get_daily_return(df, debug)
...
def BaselineStats( baseline_start, baseline_end , baseline_ticker = '^DJI'):
...
def BackTestPlot(account_value, baseline_start, baseline_end, baseline_ticker = '^DJI'):
...
def get_daily_return(df, debug=True):
