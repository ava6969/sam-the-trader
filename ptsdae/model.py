...
def load_trained(ae):
    ...
    files = sorted(files, key=lambda x: int(x.split('_')[-2]),reverse=True)
    file = files[0]
    ae.load_state_dict(torch.load(file))
    return ae


def load(ae):
    files = glob.glob('pretrained_models/*')
    files = sorted(files, key=lambda x: int(x[-1]))
    number_of_subautoencoders = len(ae.dimensions) - 1
    for index in range(number_of_subautoencoders):
        encoder, decoder = ae.get_stack(index)
        embedding_dimension = ae.dimensions[index]
        hidden_dimension = ae.dimensions[index + 1]
...
def train(
        ...
        epoch_callback: Optional[Callable[[int, torch.nn.Module, int, float, float], None]] = None,
) -> None:
    """
    ...
    for epoch in range(epochs):
        ...
        if epoch_callback is not None:
            autoencoder.eval()
            epoch_callback(epoch, autoencoder, model_index, loss_value, validation_loss_value)
            autoencoder.train()
